translations = {

    "DA": {

        "title":
            "Brandbeskyttelse af stålkonstruktioner",

        "next":
            "Næste →",

        "previous":
            "← Forrige",

        "new_calculation":
            "Ny beregning",

        "new_project":
            "Nyt projekt",

        "materials":
            "Materialeforbrug",

        "profile_length":
            "Profil længde (meter)",

        "waste_percent":
            "Spild (%)",

        "select_variant":
            "Vælg lagopbygning",

        # ---------------------------------------------------
        # SIDEBAR
        # ---------------------------------------------------

        "calculations":
            "Beregninger",

        "download_all_calculations":
            "Download alle beregninger",

        # ---------------------------------------------------
        # TABS
        # ---------------------------------------------------

        "tab_profile":
            "Profil",

        "tab_cladding":
            "Inddækning",

        "tab_fire":
            "Brand",

        "tab_result":
            "Resultat",

        # ---------------------------------------------------
        # TAB 1
        # ---------------------------------------------------

        "select_profile_category":
            "Vælg profilkategori",

        "select":
            "Vælg",

        "select_profile_type":
            "Vælg profiltype",

        "select_profile_size":
            "Vælg profilstørrelse",

        "no_profiles_found":
            "Ingen profiler fundet",

        # ---------------------------------------------------
        # PROFILE CATEGORIES
        # ---------------------------------------------------

        "h_profiles":
            "H-profiler",

        "i_profiles":
            "I-profiler",

        "u_profiles":
            "U-profiler",

        "shs_hot":
            "Kvadratiske rør varmvalsede",

        "shs_cold":
            "Kvadratiske rør koldvalsede",

        "rhs_hot":
            "Rektangulære rør varmvalsede",

        "rhs_cold":
            "Rektangulære rør koldvalsede",

        "chs_medium":
            "Cirkulære rør middelsvære",

        "chs_heavy":
            "Cirkulære rør svære",

        "other_profiles":
            "Andre profiler",

        # ---------------------------------------------------
        # OTHER PROFILES
        # ---------------------------------------------------

        "profile_name_optional":
            "Profilnavn (valgfri)",

        "select_method":
            "Vælg metode",

        "enter_apv":
            "Indtast Ap/V",

        "calculate_apv":
            "Beregn Ap/V",

        "enter_apv_ratio":
            "Indtast Ap/V forhold (m²/m³)",

        "enter_perimeter":
            "Indtast indvendig omkreds (mm)",

        "enter_area":
            "Indtast stålets tværsnitsareal (mm²)",

        "calculate":
            "Beregn",

        "invalid_numbers":
            "Indtast gyldige tal",

        "calculated_apv":
            "Beregnet Ap/V",

        "invalid_apv":
            "Indtast gyldig Ap/V værdi",

        "special_profile":
            "Specialprofil",

        # ---------------------------------------------------
        # TAB 2
        # ---------------------------------------------------

        "select_cladding_type":
            "Vælg inddækningstype",

        "select_cladding_sides":
            "Vælg antal sider med inddækning",

        "clamping_solution":
            "Klammeløsning",

        "beam_or_phl_profile":
            "Bjælkeprofil eller PHL profil",

        "one_side_not_possible":
            "1 side ikke muligt",

        "two_sides_not_possible":
            "2 sider ikke muligt",

        "three_sides_not_possible":
            "3 sider ikke muligt",

        # ---------------------------------------------------
        # TAB 3
        # ---------------------------------------------------

        "fire_requirements":
            "Brandkrav",

        "select_fire_protection_time":
            "Vælg brandbeskyttelsestid",

        "enter_design_steel_temperature":
            "Indtast dimensionerende ståltemperatur (°C)",

        "temperature_must_be_integer":
            "Temperatur skal være et helt tal",

        "temperature_must_be_between":
            "Temperatur skal være mellem 350 og 750 °C",

        # ---------------------------------------------------
        # TAB 4
        # ---------------------------------------------------

        "calculated_profile_ratio_label":
            "Beregnet profilforhold Ap/V",

        "profile_must_be_clad_with":
            "Profil skal inddækkes med",

        "fill_all_steps_to_see_result":
            "Udfyld alle trin for at se resultat",

        "download_this_calculation":
            "📄 Download denne beregning",

        "project_information":
            "Projektoplysninger",

        "project":
            "Projekt",

        "prepared_by":
            "Udarbejdet af",

        "company":
            "Firma",

        "date":
            "Dato",

        "description":
            "Beskrivelse",

        "update_calculation":
            "🔄 Opdater beregning",

        "add_calculation":
            "➕ Tilføj beregning",

        "calculation_saved":
            "Beregning gemt",

        "material_artnr": "ART.NR.",
        "material_dbnr": "DB NR",
        "material_manufacturer": "PRODUCENT",
        "material_description": "BESKRIVELSE",
        "material_consumption": "FORBRUG PR. LBM",
        "material_unit": "ENHED",
        "material_waste": "SPILDPROCENT",
        "material_total": "SAMLET FORBRUG",
        "material_co2": "SAMLET CO2 (kg CO2e)",
        "materials_header": "Materialeforbrug",
        "profile_length": "Profil længde (meter)",
        "select_layer_build_up": "Vælg lagopbygning",

        "waste_percent": "Spild (%)",

        # ---------------------------------------------------
        # PDF
        # ---------------------------------------------------

        "sides":
            "sider",

        "minutes":
            "minutter",

        "mm":
            "mm",

        "knauf_fireboard":
            "Knauf Fireboard",
        "page":
            "Side",
    },

    "EN": {

        "title":
            "Fire protection of steel structures",

        "next":
            "Next →",

        "previous":
            "← Previous",

        "new_calculation":
            "New calculation",

        "new_project":
            "New project",

        "materials":
            "Materials",

        "profile_length":
            "Profile length (meters)",

        "waste_percent":
            "Waste (%)",

        "select_variant":
            "Select layer build-up",

        # ---------------------------------------------------
        # SIDEBAR
        # ---------------------------------------------------

        "calculations":
            "Calculations",

        "download_all_calculations":
            "Download all calculations",

        # ---------------------------------------------------
        # TABS
        # ---------------------------------------------------

        "tab_profile":
            "Profile",

        "tab_cladding":
            "Cladding",

        "tab_fire":
            "Fire",

        "tab_result":
            "Result",

        # ---------------------------------------------------
        # TAB 1
        # ---------------------------------------------------

        "select_profile_category":
            "Select profile category",

        "select":
            "Select",

        "select_profile_type":
            "Select profile type",

        "select_profile_size":
            "Select profile size",

        "no_profiles_found":
            "No profiles found",

        # ---------------------------------------------------
        # PROFILE CATEGORIES
        # ---------------------------------------------------

        "h_profiles":
            "H profiles",

        "i_profiles":
            "I profiles",

        "u_profiles":
            "U profiles",

        "shs_hot":
            "Hot rolled square hollow sections",

        "shs_cold":
            "Cold rolled square hollow sections",

        "rhs_hot":
            "Hot rolled rectangular hollow sections",

        "rhs_cold":
            "Cold rolled rectangular hollow sections",

        "chs_medium":
            "Medium circular hollow sections",

        "chs_heavy":
            "Heavy circular hollow sections",

        "other_profiles":
            "Other profiles",

        # ---------------------------------------------------
        # OTHER PROFILES
        # ---------------------------------------------------

        "profile_name_optional":
            "Profile name (optional)",

        "select_method":
            "Select method",

        "enter_apv":
            "Enter Ap/V",

        "calculate_apv":
            "Calculate Ap/V",

        "enter_apv_ratio":
            "Enter Ap/V ratio (m²/m³)",

        "enter_perimeter":
            "Enter internal perimeter (mm)",

        "enter_area":
            "Enter steel cross-sectional area (mm²)",

        "calculate":
            "Calculate",

        "invalid_numbers":
            "Enter valid numbers",

        "calculated_apv":
            "Calculated Ap/V",

        "invalid_apv":
            "Enter valid Ap/V value",

        "special_profile":
            "Special profile",

        # ---------------------------------------------------
        # TAB 2
        # ---------------------------------------------------

        "select_cladding_type":
            "Select cladding type",

        "select_cladding_sides":
            "Select number of sides with cladding",

        "clamping_solution":
            "Clamping solution",

        "beam_or_phl_profile":
            "Beam profile or PHL profile",

        "one_side_not_possible":
            "1 side not possible",

        "two_sides_not_possible":
            "2 sides not possible",

        "three_sides_not_possible":
            "3 sides not possible",

        # ---------------------------------------------------
        # TAB 3
        # ---------------------------------------------------

        "fire_requirements":
            "Fire requirements",

        "select_fire_protection_time":
            "Select fire protection time",

        "enter_design_steel_temperature":
            "Enter design steel temperature (°C)",

        "temperature_must_be_integer":
            "Temperature must be an integer",

        "temperature_must_be_between":
            "Temperature must be between 350 and 750 °C",

        # ---------------------------------------------------
        # TAB 4
        # ---------------------------------------------------

        "calculated_profile_ratio_label":
            "Calculated profile ratio Ap/V",

        "profile_must_be_clad_with":
            "Profile must be clad with",

        "fill_all_steps_to_see_result":
            "Fill all steps to see results",

        "download_this_calculation":
            "📄 Download this calculation",

        "project_information":
            "Project information",

        "project":
            "Project",

        "prepared_by":
            "Prepared by",

        "company":
            "Company",

        "date":
            "Date",

        "description":
            "Description",

        "update_calculation":
            "🔄 Update calculation",

        "add_calculation":
            "➕ Add calculation",

        "calculation_saved":
            "Calculation saved",
        "material_artnr": "ART.NO.",
        "material_dbnr": "DB NO",
        "material_manufacturer": "MANUFACTURER",
        "material_description": "DESCRIPTION",
        "material_consumption": "CONSUMPTION PER LM",
        "material_unit": "UNIT",
        "material_waste": "WASTE %",
        "material_total": "TOTAL CONSUMPTION",
        "material_co2": "TOTAL CO2 (kg CO2e)",
        "materials_header": "Materials",
        "profile_length": "Profile length (meters)",
        "select_layer_build_up": "Select layer build-up",

        "waste_percent": "Waste (%)",

        # ---------------------------------------------------
        # PDF
        # ---------------------------------------------------

        "sides":
            "sides",

        "minutes":
            "minutes",

        "mm":
            "mm",

        "knauf_fireboard":
            "Knauf Fireboard",
        "page":
            "page",
    }
}

