...
def ensure_bytes(data):
    """Return raw bytes for download data that may be a BytesIO object."""
...
def trigger_browser_download(data, file_name, mime):
    """Trigger a browser download from generated bytes via a hidden anchor."""
    encoded_data = base64.b64encode(ensure_bytes(data)).decode("ascii")
    ...
    components.html(
        ...
        (() => {{
            ...
            anchor.href = {json.dumps(data_uri)};
            anchor.download = {json.dumps(file_name)};
...
with st.sidebar:
    ...
    if st.session_state.calculations:
        ...
        auto_download_project_package = False
        ...
        if st.button(
            ...
            auto_download_project_package = project_package is not None

        if auto_download_project_package:
